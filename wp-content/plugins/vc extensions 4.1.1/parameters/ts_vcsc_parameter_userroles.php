        ...
        class TS_Parameter_UserRoles {
            ...
            function wpuserroles_settings_field($settings, $value) {
                ...
                $output .= '<div class="ts-custompost-categories-holder">';
                    ...
                    $output .= '</select>';
                    $output .= '<span style="font-size: 10px; margin-bottom: 20px; width: 100%; display: block; text-align: justify;">' . __( "Click on a name in 'Available User Roles' to add role to element; click on a name in 'Allowed User Roles' to remove from element.", "ts_visual_composer_extend" ) . '</span>';
                $output .= '</div>';
                return $output;
            }
        }
